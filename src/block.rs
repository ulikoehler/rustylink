...
pub fn parse_array_property_value(node: Node) -> Option<String> {
        ...
        match child.tag_name().name() {
            ...
            "Array" => {
                if let Some(prop_name) = child.attribute("PropName")
                    && let Some(value) = parse_array_property_value(child)
                {
            ...
            "PortProperties" => {
                let mut next_port_indices: std::collections::HashMap<String, u32> =
